...
One-time setup on the Cloudflare account, then `bun run deploy` for every change. Nothing here has been run yet; the Worker has only been exercised locally under `wrangler dev`.
...
```sh
cd worker
bunx wrangler r2 bucket create clipshare
bunx wrangler r2 bucket create clipshare-dev          # optional, used by wrangler dev --remote
bunx wrangler d1 create clipshare                     # prints a database_id
```
...
```sh
bunx wrangler secret put OWNER_TOKEN_SHA256 --env production   # paste the hex from step 1
